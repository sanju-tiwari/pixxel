...
import { Camera, CheckCircle, Icon, Info, Mountain, Sparkles, User, Wand2 } from 'lucide-react';
...
function EditingControl({project}) {
  const { canvasEditor, setprocessingMessage } = useCanva();
...
const buildRetouchUrl = (imageUrl , presskey)=>{

    const present = RETOUCH_PRESETS.find((p)=> p.key === presskey)
    if(!imageUrl || !present) return imageUrl    
    ...
    if (existingQuery) {
      ...
      if (existingTr) {
        return `${baseUrl}?tr=${existingTr},${present.transform}`;
      }
    }
    return `${baseUrl}?tr=${present.transform}`;
  }
  ...
  return (
    ...
    <h3 className="text-sm font-medium text-white mb-3">
        ...
        <div className=' grid grid-cols-2 gap-3' >
              {RETOUCH_PRESETS.map((edit)=>{
                const Icon = edit.icon
                const isSelected = selectedPreset === edit.key
                return (
                   <div key={edit.key} className={`relative p-4 rounded-lg border cursor-pointer transition-all ${isSelected ? "border-cyan-400 bg-cyan-400/10 ":" border-white bg-slate-700/30 hover:border-white/40 " }`} onClick={()=> setSelectedPreset(edit.key) } >
                    ...
                    <p className="text-white/70 text-xs">{edit.description}</p>
                    </div>
            {isSelected && (
